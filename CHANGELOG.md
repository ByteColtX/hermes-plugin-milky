...
- **BREAKING**：未配置、空环境变量或原生 `allowed_chats: []` 现在会拒绝全部普通入站；高优先级空 settings 仍会遮蔽低优先级环境配置。若需全部放行，请显式设置 `group:*` 和 `dm:*`。
- 回滚到旧版本前，先停止接收入站并设置经审阅的非空受限名单；若需全部关闭，请在 Hermes 中停用平台。旧版本会把空白名单解释为全部放行，直接回滚可能扩大入站范围。

### 新增

- 新增由 Hermes core 授权的 `/milky allowlist help/list/add/del` 在线管理；`remove` 是 `del` 的别名。持久化核验后新规则立即生效，新增群在首次获准入站时按需同步群状态。
- 新增只读 `/milky status`，显示当前插件、SSE、运行时长、白名单数量和配置一致性；不主动探测网络、不枚举规则，无法确认的信息显示为未知或不可用。
- 增加统一的中文命令帮助与结果回执，包括 `/milky help`、贴纸维护帮助和 `sticker remove` 别名；贴纸命令回执改为用户可读文本，Web 与 Agent Tool 的结构化接口保持不变。

### 验证与边界

- 全量测试：`1392 passed, 3 skipped`；跳过项需要 Hermes host 或显式开启真实集成。
- Ruff、格式检查、`uv build`、Dashboard build/check/test（3 passed）、`git diff --check` 和 `openspec validate --all --strict`（34 项通过）均通过。
- 本地真实 Hermes 命令分发契约通过且未连接 Milky；实际 QQ 回执的逐项结果没有纳入本次自动化验证。
