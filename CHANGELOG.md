...
- **BREAKING**：空/缺省入站白名单改为全部普通入站拒绝。全部放行需显式 group:*、dm:*；settings 空列表遮蔽环境。
- 新增由 Hermes core 授权的纯文本 /milky allowlist list/add/del/help，无子命令时显示帮助，remove 为 del 等价别名；核验持久设置后在线生效，完整连接读取最新规则。
- 白名单回执统一为中文提示，保留原配置来源名称；list 完整输出且不支持分页。管理命令不查询群状态，新增授权群在首次获准入站时按需准备状态。
- 实际撤销清理插件未交接状态，保留已接受任务及出站禁言状态。Web 保存仍需重新加载。
- 回滚旧版本前停止接收并使用经审阅的非空受限列表，全部关闭时在宿主停用平台；旧版本空列表会放大入站权限。
