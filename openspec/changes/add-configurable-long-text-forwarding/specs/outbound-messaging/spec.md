...
#### Scenario: Forward 节点使用已确认身份

- **WHEN** 系统构造自动合并转发节点
- **THEN** 每个节点 SHALL 使用已确认的 Bot `user_id` 和 `sender_name`
- **AND** 系统 SHALL 不从入站正文、forward ID 或未确认字段猜测身份
