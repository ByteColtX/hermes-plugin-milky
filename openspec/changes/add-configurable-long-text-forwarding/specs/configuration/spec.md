...
启动配置 MUST 解析可选的 `MILKY_LONG_TEXT_FORWARD_THRESHOLD`。未配置时值 MUST 为 `0`；值 MUST 是十进制整数且范围为 `0` 至 `4000`（含边界）。值为 `0` 时 MUST 禁用超长文本合并转发；正值 MUST 表示当一次出站文本的可见规范化长度超过该阈值时选择合并转发。配置 MUST 在启动时一次性解析并保存在运行时配置中，且 MUST 出现在配置文档和不含凭证的配置摘要中。
...
- **WHEN** 配置值分别为 `1` 或 `4000`
- **THEN** 启动 SHALL 接受对应配置
- **AND** `1` SHALL 表示大于 1 个字符时可选择合并转发，`4000` SHALL 表示大于 4000 个字符时可选择合并转发

#### Scenario: 非法阈值拒绝启动

- **WHEN** 配置为空、不是十进制整数、为负数或大于 `4000`
