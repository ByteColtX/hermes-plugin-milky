...
- 不在普通消息或连接初始化中调用视觉模型；视觉调用只由到达插件 command handler 的显式 `sticker add`、`add --dry-run` 或 `reanalyze` 参数驱动。本 change 不承诺 handler 调用一定来自 Milky friend/group，也不承诺宿主超时后 core 内部任务一定已回收。视觉调用失败或结果结构非法时新增候选继续留在 inbox；合法 `is_sticker=false` 时移动到 `junk/`，不自动再次处理。已入库 `reanalyze` 返回 `false` 时保留原条目，不移动文件。
- 不提供 Agent 收藏/删除 Tool、跨 chat 作用域、远程 URL 导入、任意路径导入、图片发送或相似度搜索。
- 不把视觉失败或结构非法的 inbox 原文件移动、覆盖或删除作为隐含副作用；合法 `is_sticker=true` 的文件会移动到 library，合法 `is_sticker=false` 的文件会移动到 `junk/`。不提供独立处理命令或审批状态机。
...
├── stickers/
│   ├── inbox/                 # 操作者投放、等待处理的原文件
│   ├── library/               # is_sticker=true 后移动到此处的 content-addressed 文件
│   └── junk/                  # is_sticker=false 后隔离保存的原文件
...
### 6. 视觉辅助使用 core 的独立分析路径，不进入主 Agent 上下文
...
tool result string
  ...
  -> 校验 emotion/tags/description/is_sticker
```

成功 envelope 的 `analysis` 可能带有缩放或裁剪提示前缀，并可能同时有 `scale_note` 字段；不得用任意方括号正则吞掉模型正文，只能去除已确认的 core 前缀。`success=false`、存在 `error`、缺少或为空的 `analysis`、外层 JSON 非对象、内层 JSON 非对象或任何解析/字段校验失败，均归类为 `visual_unavailable`，不得移动文件或入库。模型只负责返回内层 JSON；提示词中的“只返回一个 JSON 对象”不等于 `vision_analyze_tool` 的最终返回只有这一层。新增候选的内层对象合法且 `is_sticker=false` 时归类为 `junk` 并移动到 `junk/`；已入库条目执行 `reanalyze` 返回 `false` 时保留原条目并报告 `not_sticker`。只有合法且 `is_sticker=true` 才能新增或更新 library 元数据。
...
内层 JSON 超出边界、夹带自由格式正文或不是对象，或外层 envelope 解析失败，都视为本次视觉打标失败；该候选不移动、不入库，保留在 inbox 等待下一次命令重试。
...
cleanup 将库状态分成三类：有 entry 引用且文件存在（正常）、有 entry 但文件缺失（保留 metadata 并报告 missing_file）、无 entry 引用的完整文件或受控临时文件（可在正常模式删除）。它不删除 inbox 或 `junk/`，也不通过猜测补建 entry。

reindex 的扫描、索引替换、缺失引用和 orphan 语义详见第 10 节；cleanup 不借助 reindex 猜测或补建可见 entry。

### 10. reindex 只重建库文件技术索引
...
命令层只向用户返回固定的 `invalid_input`、`batch_deferred`、`rejected`、`duplicate`、`junk`、`not_sticker`、`missing_file`、`storage_error`、`visual_unavailable`、`unsupported` 等分类及低基数计数。logger 不写原始参数、路径、URL、图片 bytes、异常正文或凭证。视觉失败只影响当前候选或当前 reanalyze，不阻塞普通 SSE、Hermes turn 或其他维护命令；store 或 metadata 提交失败只结束本次命令，不修改 SSE、Gate、Will、buffer、Hermes handoff、reply cost 或 outbound sender 状态。

## Risks / Trade-offs

- **[视觉模型判断不稳定]** 视觉模型可能误判主情绪、检索标签或 `is_sticker`。→ `--dry-run` 先展示完整结果，`is_sticker=false` 只进入 `junk/` 而不是永久删除，`edit` 允许修正已入库元数据，`reanalyze` 返回 `false` 时保留原条目；需要再次处理 junk 文件时由操作者手动移回 inbox。
...
- **[文件移动与数据库事务不是同一原子域]** 数据库成功和文件移动不能由单一事务覆盖。→ `is_sticker=true` 先把已校验的原文件原子移动到 content-addressed library，再提交 entry；数据库失败时由受控 orphan cleanup/reindex 回收无引用库文件，移动失败时源文件仍在 inbox；`is_sticker=false` 移动失败同样保留源文件。
- **[junk 文件长期占用磁盘]** `junk/` 不由 cleanup 自动删除，可能长期积累。→ 该目录只是简单隔离区，操作者可直接删除文件；需要再次处理时手动移回 inbox，不增加状态机。
...
2. 操作者将人工筛选后的图片包复制到 inbox，执行 `/milky sticker add --dry-run`；命令先做文件校验和 hash 去重，再显示每张候选的主情绪、中文检索标签、短描述和 `is_sticker` 判定。确认标签质量后执行正式 `add`；合法且 `is_sticker=true` 的原文件移动到 library，合法且 `is_sticker=false` 的原文件移动到 `junk/`，视觉失败或结构非法的条目留在 inbox，等待下次命令增量重试。已入库条目可用 `edit` 手动纠正，或用 `reanalyze <sticker_id>` 重新生成视觉基线。
