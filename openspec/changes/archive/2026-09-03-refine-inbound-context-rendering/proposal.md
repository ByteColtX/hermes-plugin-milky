...
- 将全体提及显示为 `@全体成员`，并让 image、file、forward、market_face placeholder 显式标注字段名和值。
- 按 segment 类型提供稳定、可读的占位符；`market_face` 保留 `summary`，`xml` 使用 `NOT SUPPORTED`，未知 segment 继续不进入正文。
...
- forward 入站只展示 `[forward:forward_id=<forward_id>]`，trigger 阶段不自动调用 `get_forwarded_messages`；后续由显式 QQ Tool 按需查询。
