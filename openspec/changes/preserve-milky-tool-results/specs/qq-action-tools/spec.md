...
校验、业务字段校验、敏感字段过滤、容器转换、JSON 重建、摘要或错误分类替换，也不得把
结果写入普通入站上下文。
...
- **WHEN** 查询 Action 返回协议拒绝、非成功 HTTP 状态、JSON 数组、显式 `null` 或缺少历史最小字段的响应体
- **THEN** Tool 调用方 SHALL 收到该响应体的原始内容
- **AND** 工具 SHALL 不返回插件自有 `rejected`、`http_error` 或 `malformed` 替代结果
...
或协议状态重建结果；未取得远端响应体时，工具 MUST 返回 `transport_unknown`，不得自动重发
或把未知结果伪装成成功。
...
工具 MUST 在网络前区分 `invalid_input`、未绑定或已关闭的 `unsupported`，并在未取得远端响应体
时保留 `transport_unknown`；只要取得响应体，插件 MUST 将其作为不透明结果原样交付，不得根据
HTTP 状态、Milky envelope、`data` 结构或 JSON 可解析性返回 `rejected`、`http_error`、`malformed`
或成功摘要。工具完成调用或得到本地/传输分类时，插件 SHALL 使用标准 logger 记录一个低基数
的 `event=milky.tool` 结果事件，至少包含工具名称、远端响应分类或固定结果分类、已知 HTTP
状态码和 `duration_ms`；该日志不是 Tool raw 响应的副本。
...
- **AND** 日志 SHALL 只记录 Tool 名称、远端响应分类、状态码和耗时

#### Scenario: 成功响应缺少历史最小结构

- **WHEN** 查询响应缺少历史要求的字段、管理响应的 `data` 非空、或响应体不是 JSON object
- **THEN** Tool 调用方 SHALL 收到该响应体的原始内容
- **AND** SHALL 不返回 `malformed` 替代结果或报告假成功

#### Scenario: 成功 data 结构缺失

- **WHEN** 查询工具成功响应缺少历史要求的字段，或管理工具的 data 不是空对象
...
- **WHEN** Agent 在工具 client 未绑定或已关闭，或请求未取得远端响应体
- **THEN** 工具 SHALL 在网络访问前或传输结束后返回对应的 `unsupported` 或 `transport_unknown`
...
`get_friend_info` 成功时 MUST 向 Tool 调用方返回远端响应体的完整原始内容。插件只允许在网络
访问前校验 `user_id` 和在传输层确认是否取得响应体；不得擅自规定或改写好友资料内部字段，
也不得对响应体执行 envelope/data 校验、敏感字段过滤、容器转换、JSON 重建或本地状态投影。
只要取得响应体，即使其表示协议失败、HTTP 错误、`data` 缺失、为 `null`、为数组或不是 object，
工具也 MUST 原样交付；未取得响应体时才返回既有传输分类。查询结果不得自动写入普通入站上下文、
本地好友状态或 Agent 指令。
