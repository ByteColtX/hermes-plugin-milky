...
Milky plugin MAY 在现有 `milky` toolset 中注册固定的 `sticker_categories`、`sticker_search`、`sticker_send` 和 `sticker_forget`。自动 sticker collection SHALL 是内部旁路，不得注册为 Agent Tool；这些工具 SHALL 使用稳定的本地 library 契约，不得伪装成未确认的 Milky operationId，不得开放任意 Action catalog，也不得因普通消息、关键词、Will 或系统事件隐式执行发送、删除或维护动作。
...
- **AND** 发送和删除状态变化 SHALL 只能来自显式固定工具或受权限控制的维护入口

### Requirement: Sticker ToolSpec arguments MUST be strict and bounded

`sticker_categories` MUST 不接受参数；`sticker_search` MUST 只接受可选的 `category`、`keyword`、`tags` 和 `limit`；`sticker_send` MUST 只接受可选的 `sticker_id`、`category`、`tags` 和 `index`；`sticker_forget` MUST 只接受当前搜索结果中的 `sticker_id`。所有 schema MUST 拒绝额外字段、空字符串、布尔冒充整数、超长文本和超出上限的列表；非法输入 SHALL 在读取图片或发送网络 Action 前返回 `invalid_input`。

#### Scenario: Unknown ToolSpec field is rejected

- **WHEN** Agent 向任一 sticker tool 传入未声明字段
- **THEN** 工具 SHALL 返回 `invalid_input`
- **AND** SHALL 不改变数据库、文件或远端会话

#### Scenario: Sticker ID is treated as opaque

- **WHEN** Agent 传入非当前搜索结果、格式异常或不可见的 `sticker_id`
- **THEN** 工具 SHALL 返回 `sticker_not_found` 或 `invalid_input`
- **AND** SHALL 不把该值解释为路径、URL、资源 URI 或 Milky Action 名称

#### Scenario: Forget is limited to current scope

- **WHEN** Agent 使用当前会话可见的 `sticker_id` 调用 `sticker_forget`
- **THEN** 工具 SHALL 只删除当前作用域的 entry
- **AND** SHALL 不删除其他会话的 entry 或仍被引用的共享 asset

### Requirement: Sticker send ToolSpec MUST account for one network side effect

`sticker_send` 在选定条目后 MUST 最多调用一次现有 Milky native image send；成功、拒绝、未知和不支持结果 SHALL 保持既有出站错误分类。工具不得通过多次重试规避未知结果，也不得在工具结果中伪造成功消息序号。

#### Scenario: Native send succeeds

- **WHEN** 当前可见 sticker 被选中且 native image send 返回可确认成功
- **THEN** Tool SHALL 返回成功状态和受限 sticker 元数据
- **AND** library 使用统计 SHALL 只更新一次

#### Scenario: Native send has unknown result

- **WHEN** native image send 已进入网络边界但结果未知
- **THEN** Tool SHALL 返回 `transport_unknown` 或对应既有分类
- **AND** SHALL 不重发、不计数且不报告成功

#### Scenario: Send target is not current session

- **WHEN** 工具参数或内部状态试图把 sticker 发往其他群、好友、temp 目标或未确认目标
- **THEN** Tool SHALL 在网络 Action 前返回 `unauthorized` 或 `invalid_input`
