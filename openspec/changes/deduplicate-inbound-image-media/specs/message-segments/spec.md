...
`forward_id` 与 reply 目标也只能作为延迟引用保存，供 trigger 阶段使用。`media_resource_references` 的协议查询使用
已确认的 `get_resource_temp_url`；group file 使用 `get_group_file_download_url(group_id, file_id)`，private file 使用
`get_private_file_download_url(user_id, file_id, file_hash, ...)`，不得把 file 套用到 resource Action。
