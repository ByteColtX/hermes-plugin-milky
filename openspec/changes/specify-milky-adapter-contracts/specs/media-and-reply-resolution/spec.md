...
在消息真正触发 Hermes turn 时补全 `media_resource_references`（image、record、video）、
`file_attachment_references`（入站 file）、forward 和 reply 引用；只有成功 materialize
为本地附件的结果才能进入 Hermes `MessageEvent`，下载安全、缓存、权限与路径控制仍由
Hermes 公共附件边界负责。
