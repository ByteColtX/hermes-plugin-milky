...
- **AND** SHALL 保持普通入站全部阻止，同时允许后续由 core 允许的管理流程按需准备群状态
...
### Requirement: 热授权和管理来源群按需准备状态

新增授权范围 SHALL 在发布前完成当前相关群的身份确认与 Bot 成员状态准备；管理来源群缺少状态时 SHALL 仅在 Hermes core 放行并完成语法校验后、名单读写前准备；插件 SHALL 不读取管理员名单决定是否查询。具体新增群不在已确认群列表中时 SHALL 刷新确认其归属，仍无法确认则拒绝本次修改；新增 group:* SHALL 为当前群列表中尚需准备的授权群取得状态。查询失败 MUST 保持未授权候选不生效；成功取得 muted 状态属于已确认状态，目标授权可以保存，但普通入站仍须遵守禁言门禁。

按需准备 SHALL 复用既有只读查询、冷却与有界并发规则；期间发生的禁言事件 MUST 被保留，不能由较旧查询或状态发布覆盖。来源管理例外 SHALL 不自动改变普通入站范围。移除入站授权 SHALL 不让群状态被解释成 unmuted，不删除既有发送路径仍需使用的状态。

#### Scenario: 新增未跟踪群

- **WHEN** core 允许的调用者从可用来源添加一个已确认 Bot 所在、但尚未跟踪的群
- **THEN** 系统 SHALL 在发布授权前取得该群的 Bot 成员状态
- **AND** 查询失败 SHALL 不保存或发布候选，成功但禁言 SHALL 明示授权与可发言不同

#### Scenario: 新增群通配符

- **WHEN** core 允许的调用者添加 group:*，当前群列表中有多个尚未准备状态的群
- **THEN** 系统 SHALL 完成所需群状态准备后发布整份候选
- **AND** 任一必要准备失败 SHALL 保留旧名单，不发布部分成功范围

#### Scenario: 查询期间收到禁言事件

- **WHEN** 群状态准备期间收到该群的成员或全体禁言事件
- **THEN** 发布后的快照 SHALL 保留更新的事件事实
- **AND** SHALL 不因旧查询返回而把新禁言解释为可发言

#### Scenario: 只为管理查询来源群

- **WHEN** 一个未放行群仅因 core 允许的 list 指令完成状态准备
- **THEN** 该群 SHALL 保持原普通入站授权结果
- **AND** 后续普通消息 SHALL 不因状态已缓存而绕过白名单

#### Scenario: 刷新未知群不隐式建立跟踪

- **WHEN** 既有普通状态刷新请求指向尚未跟踪的群
- **THEN** 系统 SHALL 保持该刷新请求不成功，不据此隐式纳入跟踪或扩大白名单
- **AND** core 放行的管理流程 SHALL 可通过受控准备操作确认该群并建立状态

#### Scenario: 未跟踪群准备失败或被取消

- **WHEN** 群已确认归属并开始受控准备，但成员查询失败或操作被取消
- **THEN** 该群 SHALL 不被视为已确认可发言，管理候选 SHALL 不保存或发布
- **AND** 查询期间的事件 SHALL 保留，后续准备重试 SHALL 遵守冷却与并发限制
