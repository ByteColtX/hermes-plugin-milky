...
- 为白名单管理指令设置不依赖发送者角色的路由例外，允许从未放行会话进入 Hermes core 命令分发；core 拒绝时不执行插件管理读写或群状态准备。保留 canonical、自身消息、去重与群可发送状态边界；来源群状态准备移至 core 放行之后、管理读写之前。其他命令及普通正文不享有该路由例外。
...
- 新增群授权前完成所需群身份与 Bot 成员状态准备；失败不提交本次修改。add/del 仅机械增减指定字面规则；通配符与具体条目相互独立，不展开、合并或联动删除，不生成隐式排除项；回执只报告条目增减结果。
...
- `slash-commands`：所有 slash 权限归 core，以及白名单管理指令在未放行会话的分发、可信来源与群状态准备边界。
- `mute-tracking`：空名单不扫描成员，新增授权群与管理来源群按需准备，保持禁言约束。
