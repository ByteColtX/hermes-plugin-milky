...
- [ ] 2.1 抽出仅用于 `channel_context` 的 context renderer，根据已确认的 `dm:`/`group:` chat namespace 选择模板；dm 普通记录只渲染经过现有 body 编码的正文，group 继续复用既有 header 行为；验证方式：renderer 单元测试覆盖 dm/group 分支、顺序、`None` 和边界字符。
- [ ] 2.2 将 detached batch 的历史组装接入 context renderer，并保持当前 `MessageEvent.text` 的普通消息 renderer、system event `<event ...>` 格式、ingress 排序和资源解析后正文不变；验证方式：端到端 fake Hermes 测试确认 dm `channel_context` body-only、group `channel_context` 字节级不变、当前 text 不变。
