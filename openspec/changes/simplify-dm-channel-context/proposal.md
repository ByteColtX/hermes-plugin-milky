...
当前 friend 和 group 的历史 `channel_context` 共用同一套普通消息 header。对于一对一私聊，sender、uid、message ID 和 reply 元信息通常是重复噪音，挤占了 Agent 可读上下文；群聊仍需要这些字段来区分同一群中的多个发送者。现在应将私聊历史上下文与群聊 renderer 解耦，同时保持既有单行编码和群聊行为稳定。
...
- 影响入站历史上下文的 renderer 选择和 detached batch 到 `channel_context` 的映射边界。
