...
对于 dm 普通历史消息，每条记录 MUST 只包含经过现有 body 编码规则处理的正文，不得生成
sender、uid、`msg_id`、`reply_to` 或其他普通消息 header。dm 记录仍必须保持单行格式。
group 中 `msg_id` 和 `reply_to` 没有值时 MUST 省略，并保持既有字段顺序；group 普通消息
header 和 body 之间使用一个空格。若 group 中被实际渲染的 `reply` 目标的 `sender_id`
等于当前 Bot 的 `self_id`，Agent-facing header 中的 `reply_to <reply_id>` MUST 改为
`reply_to your_previous_msg`，不得在同一个 `reply_to` 字段中展示该 Bot 消息的数字 ID。
当前消息自身的 `msg_id` MUST 继续展示真实 Milky 消息 ID（若可用）。该展示替换只适用于
交给 Agent 的 `MessageEvent.text` 和 group `channel_context`；Hermes
`MessageEvent.reply_to_message_id` 及其他内部引用字段 MUST 继续保留真实的 Milky
`message_seq`。
...
推断自引用。dm 历史普通消息不展示 reply header，但当前 dm trigger 的 `MessageEvent.text`
仍遵循现有 reply 展示和 Hermes 内部引用规则。

系统事件 MUST 使用 `<event <event_type>> <body>` 格式，不得伪装成普通消息或 segment
placeholder；该格式对 group 和 dm 均保持不变。普通消息的 `body` MUST 来自规范化消息
内容和本 change 定义的结构化占位符；系统事件的 body MUST 来自事件字段的可读渲染。
普通消息和系统事件之间 MUST 使用一个换行拼接；不得添加额外历史标题。

当前 trigger 消息 MUST 只进入本次 `MessageEvent.text`，不得进入 `channel_context`；没有
历史消息和待注入系统事件时，`channel_context` MUST 为 `None`，而不是空字符串。当前
`MessageEvent.text` 继续使用现有单行 header 和正文格式，不因 dm 历史 body-only 规则改变。

group header 中的非可信值 MUST 将尖括号、反斜杠、回车和换行编码为不会改变记录边界的
字面量；group 和 dm body 中的回车和换行也 MUST 编码为字面量 `\\n`。dm body 中的尖括号
和反斜杠继续按现有 body 规则处理，不得新增 header 专用编码。上下文 MUST NOT 包含
timestamp、dedup key、认证信息或插件本地媒体路径。

#### Scenario: 当前消息引用 Bot 时使用 Agent-facing 自引用文案

- **WHEN** 当前 friend 或 group trigger 消息包含 `reply`，且被实际渲染的 `reply.data.sender_id` 等于当前 Bot 的 `self_id`
...
- **AND** dm 历史普通消息 SHALL 不因多个 reply 生成任何普通消息 header
...
- **WHEN** detached batch 包含 group 历史和当前 group trigger，或包含 dm 历史和当前 dm trigger
- **THEN** `channel_context` SHALL 只包含对应 chat 的历史记录
- **AND** group 历史记录 SHALL 使用既有 `<sender uid ... msg_id ... reply_to ...> body` 格式
- **AND** dm 历史记录 SHALL 只包含 body，不生成普通消息 header
- **AND** 当前 trigger SHALL 只出现在 `MessageEvent.text`
...
- **AND** group 每条记录 SHALL 保留可用的 sender、uid、msg_id 和 reply_to 字段
- **AND** dm 每条记录 SHALL 只保留对应的转义正文
...
- **AND** 普通 group 记录 SHALL 使用既有消息 header，普通 dm 记录 SHALL 只使用 body
...
- **AND** 当前消息 SHALL 仍使用现有单行 header 和正文格式交给 Hermes

#### Scenario: 上下文包含边界字符

- **WHEN** group sender name 或 body 含有尖括号、反斜杠或换行
...
- **AND** body SHALL 保持现有 body 文本语义，不生成 header 或额外记录
- **AND** 原始 payload SHALL NOT 被直接拼接到上下文

#### Scenario: 上下文为空或包含非可信文本

- **WHEN** detached batch 为空，或历史消息包含非可信正文
