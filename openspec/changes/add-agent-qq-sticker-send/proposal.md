...
- Tool 不接受 `sticker_id`、`emoji_id`、`face_id`、`chat_id`、`session_id`、任意文件路径或远端媒体 URL；发送目标只从可信的 task-local Milky session context 获取。成功回执不返回内部 `sticker_id`，Agent 不能指定或读取它。
- 只读取贴纸库当前生效的 `emotion`、`tags`、`description` 字段；字段是人工修正还是视觉生成不改变匹配优先级。
- 使用插件运行时声明的本地中文分词依赖和无数值阈值的分层词法证据检索，对意图和贴纸元数据进行相关性排序；贴纸库为空时不向 Agent 暴露 `sticker_send`。不新增独立的 `sticker_search`，不调用远程搜索或模型进行发送时匹配。
...
- 返回固定的成功、无匹配、缺少会话上下文、存储失败和远端失败分类；成功结果只包含 `status=sent` 和 `message_id`。
...
- 需要按照 Hermes plugin manifest 的 `python_dependencies` 约定声明 `Pillow>=12.3.0` 和 `jieba>=0.42.1`，并在项目运行时依赖中提供同样的无上限下界约束；`jieba` 是插件正常运行时依赖，不做按需导入或可选依赖缺失分支。贴纸库为空时仍不向 Agent 暴露 `sticker_send`，但不改变依赖加载边界。
